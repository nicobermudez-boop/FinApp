  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px' }}>
        ...
        {/* Cron info */}
        <div style={{ ...cardStyle, opacity: 0.7, marginBottom: 32 }}>
...
function ChangePassword() {
  ...
  const inputStyle = {
    width: '100%', maxWidth: 320, padding: '9px 14px',
  ...
  return (
    <div>
      ...
      <button onClick={handleChange} disabled={loading || !newPass || !confirmPass} style={{
        padding: '9px 20px', borderRadius: 'var(--radius-sm)', border: 'none',
        background: 'var(--color-accent)', color: '#fff', fontSize: 13, fontWeight: 600,
        cursor: loading ? 'not-allowed' : 'pointer', opacity: (loading || !newPass || !confirmPass) ? 0.5 : 1,
        fontFamily: 'inherit',
      }}>
      ...
      {result && (
        <div style={{
          marginTop: 12, padding: '10px 14px', borderRadius: 'var(--radius-sm)', fontSize: 13,
          ...(result.type === 'success'
            ? { background: 'var(--color-income-bg)', border: '1px solid var(--color-income-border)', color: 'var(--color-income)' }
            : { background: 'var(--color-expense-bg)', border: '1px solid var(--color-expense-border)', color: 'var(--color-expense-light)' }),
        }}>
          {result.msg}
        </div>
      )}
